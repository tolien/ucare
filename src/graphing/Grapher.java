...
public class Grapher implements GraphTool, ImageGenerator
{
	...
	private JFreeChart createChart(XYDataset occupancy, XYDataset power)
	{
		...
		JFreeChart chart = ChartFactory.createTimeSeriesChart("Lab Usage Data", // title
				"Date", // x-axis label
	...
	private void createDataset(String labName, Date start, Date end,
			...
			String namePostfix, double multiplier)
	{
	...
	private void createMultiDataset(String labName, Date start, Date end,
			Map<Date, List<Double>> labData, TimeSeriesCollection dataset,
			String namePostfix, double multiplier)
	{
		// Using this rather than calling create dataset 3X means it can be done
		// in 1 pass
		...
		{
			TimeSeries s1 = new TimeSeries(labName + namePostfix + " unit "
					+ (i + 1));
			for (Date key : keys)
			{
				if (key.compareTo(start) >= 0 && key.compareTo(end) <= 0)
					s1.add(new Minute(key), labData.get(key).get(i)
							* multiplier);
			}
			dataset.addSeries(s1);
		}
			...
			int axisType)
	{
		...
		{
			Map<Date, Double> labData = dataStore.getAbsoluteOccupancy(labName,
					start, end);
			createDataset(labName, start, end, labData, occupancy, "", 1);

			if (axisType == InputGUI.SPLIT_POWER)
			{
				Map<Date, List<Double>> labPower = dataStore.getPower(labName,
						start, end);
				if (labPower != null)
					createMultiDataset(labName, start, end, labPower, power,
							" Power", multiplier);
			}
			else
			{
				Map<Date, Double> labPower = null;
				String annotation = null;
				if (axisType == InputGUI.TOTAL_POWER)
				{
				...
				else if (axisType == InputGUI.CO2)
				{
					labPower = dataStore.getCO2(labName, start, end);
					annotation = " CO2";
					axisLabel = "CO2 (ppm)";
				}
				else if (axisType == InputGUI.TEMPERATURE)
				{
					labPower = dataStore.getAverageTemperature(labName, start,
							end);
					annotation = " ¼C";
				...
				if (labPower != null)
				{
					createDataset(labName, start, end, labPower, power,	annotation, multiplier);
				}
			}
		}
	}
